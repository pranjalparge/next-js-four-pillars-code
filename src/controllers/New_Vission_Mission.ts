...
const Four_PII_About = require("../models/4pii_New_Vission_Mission");
const krislin_About = require("../models/krislin_visionMission");
const LLC_About = require("../models/LLC_vissionMission");
const saas_About = require("../models/99ideasSaas/saas_NewVissionMission");
const LLP_About = require("../models/LLp_visionmission");
...
module.exports.Add_llp = async (req, res) => {
  try {
    ...
    await data.save();
    res.json({ message: "vision_mission_LLP_data", data });
    console.log(data);
  } catch (error) {
    res.send(error);
    console.log(error);
