...
// const Four_PII_About = require("../models/4pii_event");
const Four_Event = require("../models/4pii_event")
...
module.exports.Add_4pii = async (req, res) => {
  try {
    console.log("sima");
    const data = await Four_Event.create({
        info:req.body.info,
...
module.exports.get_4pii_Data = asyncMiddleware(async (req, res) => {
  ...
  try {
    let getData = await Four_Event.find();
...
module.exports.update_4pii = asyncMiddleware(async (req, res) => {
  try {
    ...
    const data = await Four_Event.findById(req.body.id);
    if (!data) {
      res.status(400).json({ success: false, msg: "data  not exits" });
    }

    const mydata = await Four_Event.findByIdAndUpdate(
      req.body.id,
      req.body,
      {
        new: true,
...
module.exports.delete_4pii = asyncMiddleware(async (req, res) => {
  try {
    ...
    const newdata = await Four_Event.findByIdAndDelete(req.body.id);
...
//     }
//     await data.save();
//     res.json({message:"about 4pii_event data" , data:data})
//     // const data = await Four_PII_About.create({
//     //   info:req.body.info,
//     //   info_head:req.body.info_head
//     // });
